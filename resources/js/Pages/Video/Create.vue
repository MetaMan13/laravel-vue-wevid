			...
			<form @submit.prevent="storeVideo">
				...
				<div class="mt-6">
					<div class="flex flex-col">
						...
						<textarea v-model="form.description" cols="20" rows="7" placeholder="Enter a video description ( optional )"
							class="border border-gray-200 rounded-md focus:ring-transpatent focus:ring-0 focus:border-gray-300 placeholder-gray-400"></textarea>
					</div>
					...
					<div class="flex flex-col">
						...
						<div class="bg-white border border-gray-200 rounded-md hover:shadow-sm hover:bg-gray-50">
							<div class="flex justify-center">
								...
								<div v-if="form.file != null" @dragover.prevent @drop.prevent="appendDragAndDropFile($event)" class="w-full h-full">
                                    <div v-if="videoSource">
										<video ref="videoPreviewElement" class="w-full h-72 border-b border-gray-200 rounded-tl-md rounded-tr-md bg-gray-800" controls autoplay loop>
                                            <source :src="videoSource" type="video/mp4">
                                        </video>
                                        <div class="px-4 py-4">
                                            <div class="flex justify-between items-center">
                                                <h3 class="text-2xl font-semibold underline">File information</h3>
                                                <div class="">
                                                    <button @click="discartFile" :disabled="form.processing" :class="{'cursor-not-allowed' : form.processing }"
                                                            class="bg-red-500 hover:bg-red-600 text-white px-6 py-2 cursor-pointer rounded-md font-semibold flex gap-1 items-center justify-center select-none">
                                                        Discart
                                                        <trash
                                                            :width="16"
                                                            :height="16"
                                            ...
                                            <div class="flex gap-4 mt-4">
                                                <div class="w-full">
                                                    ...
                                                    <p class="text-base">{{ form.file.name }}</p>
                                                </div>
                                                <div class="w-full">
                                                    <p class="text-lg font-semibold">Duration:</p>
                                                    <p class="text-base">{{ videoDuration }} seconds</p>
                                                </div>
                                            </div>
                                        </div>
									</div>
									...
									<div class="px-4 py-4">
                                        <div>
                                            <h3 class="text-2xl font-semibold underline">Create Gif</h3>
                                        </div>
                                        <div class="flex gap-4 mt-4">
				...
				<div class="mt-6">
					<div>
						...
						<button v-show="form.progress" @click="form.cancel()" type="button"
								class="bg-red-500 hover:bg-red-600 text-white border border-gray-200 w-full py-4 rounded-md mt-4">
							Cancel
						</button>
					</div>
				</div>
	...
	methods: {
		...
		handleFileAdded()
		{
            ...
            this.$nextTick(() => {
                ...
                this.$refs.videoPreviewElement.addEventListener('loadedmetadata', loaded);
            })
		},
		discartFile()
		{
